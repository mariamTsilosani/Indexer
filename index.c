...
/*sort node recieves all the info for a new node to be inserted
 * finds a right spot to insert it in (decending order) and inserts
...
/* sortFileName recieves a file path and a linked list of fileNode structs
 * looks if the file name exists, if it exists it incriments the wordcount 
 * and moves the existing node to a wright location, if the filename doesnt 
 ...
 * at a corresponding spot (calling sortNode if neccessary)
 */
